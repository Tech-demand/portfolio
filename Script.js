...
function animParticles() {
  ...
  particles.forEach(p=>{p.update();p.draw();}); requestAnimationFrame(animParticles);
}
animParticles();
...
dots.forEach((dot, i) => {
  const radius=145; let angle=(i/dots.length)*360;
...
function animateCount(el, target, duration=1500) {
  ...
  function step(ts){ if(!startTime)startTime=ts; const p=Math.min((ts-startTime)/duration,1); el.textContent=Math.floor(p*target+(1-p)*start); if(p<1)requestAnimationFrame(step); }
...
// We try 2 APIs: alfa-leetcode-api first, fallback to leetcode-stats-api
async function fetchLCStats() {
  // API 1: alfa-leetcode-api (most detailed)
  const url1 = `https://alfa-leetcode-api.onrender.com/${LC_USER}/solved`;
  const url2 = `https://alfa-leetcode-api.onrender.com/userProfile/${LC_USER}`;
  const url3 = `https://leetcode-stats-api.herokuapp.com/${LC_USER}`;

  try {
    const [solvedRes, profileRes] = await Promise.all([
      fetch(url1, { signal: AbortSignal.timeout(8000) }),
      fetch(url2, { signal: AbortSignal.timeout(8000) })
    ]);

    if (!solvedRes.ok || !profileRes.ok) throw new Error('alfa API failed');
    ...
    return {
      ...
      totalEasy:    solved.totalEasy       || 795,
      totalMedium:  solved.totalMedium     || 1681,
      totalHard:    solved.totalHard       || 740,
      totalQ:       (solved.totalEasy||795)+(solved.totalMedium||1681)+(solved.totalHard||740),
    ...
    // Fallback API
    try {
      const r = await fetch(url3, { signal: AbortSignal.timeout(8000) });
      ...
      return {
        ...
        totalEasy:    d.totalEasy     || 795,
        totalMedium:  d.totalMedium   || 1681,
        totalHard:    d.totalHard     || 740,
        totalQ:       d.totalQuestions|| 3216,
...
function buildHeatmap(calendar) {
  ...
  for(let w=0;w<weeks;w++) {
    ...
    for(let d=0;d<7;d++) {
      ...
      cell.title = `${date.toDateString()}: ${count} submission${count!==1?'s':''}`;
...
function renderLCStats(data) {
  ...
  const pct = data.totalSolved / data.totalQ;
  const track = document.getElementById('donut-track');

  setTimeout(() => {
    track.style.strokeDasharray = `${pct*circumference} ${circumference}`;
    animateCount(document.getElementById('lc-total'), data.totalSolved);
  }, 500);

  // Ranking & acceptance
  const rankEl = document.getElementById('lc-ranking');
  const accEl  = document.getElementById('lc-acceptance');
  if(data.ranking)        rankEl.textContent = '#' + data.ranking.toLocaleString();
  else                    rankEl.textContent = 'N/A';
  if(data.acceptanceRate) accEl.textContent  = data.acceptanceRate.toFixed(1) + '%';
  else                    accEl.textContent  = 'N/A';

  // Difficulty
  setTimeout(() => {
    // Easy
    animateCount(document.getElementById('lc-easy'), data.easySolved);
    document.getElementById('lc-easy-total').textContent = '/ ' + data.totalEasy;
    document.getElementById('lc-easy-bar').style.width = (data.easySolved/data.totalEasy*100).toFixed(1)+'%';
    // Medium
    animateCount(document.getElementById('lc-medium'), data.mediumSolved);
    document.getElementById('lc-medium-total').textContent = '/ ' + data.totalMedium;
    document.getElementById('lc-medium-bar').style.width = (data.mediumSolved/data.totalMedium*100).toFixed(1)+'%';
    // Hard
    animateCount(document.getElementById('lc-hard'), data.hardSolved);
    document.getElementById('lc-hard-total').textContent = '/ ' + data.totalHard;
    document.getElementById('lc-hard-bar').style.width = (data.hardSolved/data.totalHard*100).toFixed(1)+'%';
  }, 600);
